...
export function looksMachineMade(
	...
	if (!email) return false;
	if (normalise(firstName).length <= 1) return true;
	if (isDerivedName(email, firstName, lastName)) return true;

	const handle = normalise(email.split("@")[0] ?? "");
	const joined = normalise(`${firstName}${lastName ?? ""}`);
	const initialJoined = normalise(`${firstName.slice(0, 1)}${lastName ?? ""}`);
