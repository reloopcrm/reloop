...
export type RunBlock = { code: string; message: string };
...
export async function runBlocker(versionId: string): Promise<RunBlock | null> {
	const noModel = await modelUnavailable();
	if (noModel) return { code: MODEL_UNAVAILABLE, message: noModel };
	...
	try {
		...
		return error instanceof InvalidAgentManifest
			? { code: DEPENDENCY_UNAVAILABLE, message: error.message }
			: null;
	}
	...
	return {
		...
		message: `This agent cannot run yet. ${missing.join(" ")}`,
	};
}
