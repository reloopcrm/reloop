...
async function main() {
	...
	if (!JOINS_FOR_REAL) {
		console.log(
			"NOTE  Reloop joins no channel in this run. A join is permanent.",
		);
	}
	...
	if (!publicUnjoined) {
		record(
			...
			true,
			"skipped: Reloop is already in every public channel",
		);
	...
	if (!privateUnjoined) {
		record(
			...
			true,
			"skipped: no private channel is visible to Reloop",
		);
	} else if (!JOINS_FOR_REAL) {
	...
	record(
		...
		canInvite,
		canInvite
			? "a user token lets Reloop add itself to a private channel"
			: "no user token, so Reloop cannot add itself to a private channel",
	);
