...
const profile: WorkspaceProfile = {
	website: "reloop.de",
	narrative:
		"Reloop takes a startup from nothing to a SOC 2 or ISO 27001 audit by automating the evidence collection, and sells the platform on an annual subscription.",
	sections: {
...
describe("who we are", () => {
	...
	it("names us, and refuses to invent the rest", () => {
		...
		expect(markdown).toContain("Reloop");
		expect(markdown).toContain("reloop.de");
...
describe("a profile belongs to the website it was read from", () => {
	it("is ours while the website is unchanged", () => {
		expect(profileOf(profile, "reloop.de")).toBe(profile);
	});
...
describe("the website has to be somewhere a fetch can go", () => {
	it("takes a bare domain and gives back a URL", () => {
		expect(websiteUrl("reloop.de")).toBe("https://reloop.de");
		expect(websiteUrl(" WWW.Reloop.de/ ")).toBe("https://www.reloop.de");
	});

	it("keeps a scheme it can fetch, and a path that means something", () => {
		expect(websiteUrl("http://reloop.de")).toBe("http://reloop.de");
		expect(websiteUrl("https://reloop.de/uk/")).toBe("https://reloop.de/uk");
	});
