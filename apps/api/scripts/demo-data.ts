...
async function resolveOwner(db: Db): Promise<Owner> {
	...
	console.log(
		`Workspace "${workspace.name}" (/${workspace.slug}), owner ${member.user.email}.`,
	);
	return member.user;
}
...
async function writeDeals(
	...
	const base = await readReportingCurrency(db);
	for (const deal of DEALS) {
		...
		const fx = await convertToBase(db, amount, DEMO.currency, base);
...
async function main(): Promise<void> {
	...
	if (process.argv.includes("--remove")) {
		...
		const verdicts = await verdictsOf(db);
		await writeCompanies(db, owner, verdicts);
		await writeContacts(db, owner, contacts, verdicts);
		await writeDeals(db, owner, contacts);
		await writeConversations(db, owner, contacts);
		console.log(
			`Demo rows now in the database: ${JSON.stringify(await counts(db))}`,
		);
	}
