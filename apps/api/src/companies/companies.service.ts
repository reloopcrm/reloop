...
const SORTABLE: OrderByColumns<Prisma.CompanyOrderByWithRelationInput> = {
	...
	standing: (dir) => ({ standing: { sort: dir, nulls: "last" } }),
	potential: (dir) => ({ potentialBand: { sort: dir, nulls: "last" } }),
	...
	async list(input: CompanyListInput): Promise<ListResult<CompanyRow>> {
		...
		return {
			rows: rows.map((row) => ({
				...
				source: row.source,
				owner: row.owner,
				standing: readStanding(row.standing),
				potential: readPotential(row.potentialBand),
