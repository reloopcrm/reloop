...
export class MailboxSyncService {
	...
	async runDue(signal?: AbortSignal): Promise<TickSummary> {
		...
		await this.imapConnections.reconcileAll();

		try {
			await this.agent.businessSetupRequested();
		} catch (error) {
			this.logger.error(
				{ message: "Could not ask for the business setup" },
				error instanceof Error ? error.stack : String(error),
			);
		}
		try {
			await this.adoption.adoptRelevant();
		} catch (error) {
			this.logger.error(
