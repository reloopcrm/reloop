...
describe("when work is booked for later", () => {
	...
	it("says the day in words, never a date", () => {
		expect(enrichmentDueLabel(ahead(0, 6), NOW)).toEqual({
			text: "Later today",
			count: null,
		});
		expect(enrichmentDueLabel(ahead(1, 1), NOW)).toEqual({
			text: "Tomorrow",
			count: null,
		});
		expect(enrichmentDueLabel(ahead(3), NOW)).toEqual({
			text: "In {count} days",
			count: 3,
		});
	});
	...
	it("counts weeks from a fortnight, and months from two", () => {
		expect(enrichmentDueLabel(ahead(14), NOW)).toEqual({
			text: "In {count} weeks",
			count: 2,
		});
		expect(enrichmentDueLabel(ahead(30), NOW)).toEqual({
			text: "In {count} weeks",
			count: 4,
		});
		expect(enrichmentDueLabel(ahead(60), NOW)).toEqual({
			text: "In {count} months",
			count: 2,
		});
		expect(enrichmentDueLabel(ahead(90), NOW)).toEqual({
			text: "In {count} months",
			count: 3,
		});
	});
	...
	it("uses no internal vocabulary", () => {
		...
		for (const days of [0, 1, 3, 14, 30, 90, 365]) {
			const sentence = enrichmentDueLabel(ahead(days), NOW).text.toLowerCase();
			for (const word of banned) expect(sentence).not.toContain(word);
		}
	});
});
