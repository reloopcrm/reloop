...
describe("externalParticipants", () => {
	const options = {
		ourDomains: new Set(["reloop.de"]),
		ourAddresses: new Set(["lewis@reloop.de"]),
	...
	it("keeps only the other side of the conversation", () => {
		const result = externalParticipants(
			[
				person("lewis@reloop.de"),
				person("colleague@reloop.de"),
	...
	it("returns nothing for a wholly internal thread", () => {
		expect(
			externalParticipants([person("colleague@reloop.de")], options),
		).toEqual([]);
	});
	...
	it("leaves nothing to file when the deleted contact is the only outsider", () => {
		expect(
			externalParticipants(
				[person("lewis@reloop.de"), person("deleted@acme.com")],
				options,
			),
		).toEqual([]);
	});
