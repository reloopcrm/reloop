...
describe("disconnecting Google", () => {
	...
	it("keeps both sync rows when Google will not revoke the token", async () => {
		await grant(SYNC_SCOPES.join(" "), "refresh-token");
		await state.ensure(gmailRep, "gmail", { autoCreate: false });
		await state.ensure(gmailRep, "calendar", { autoCreate: true });
		...
		expect(await state.get(gmailRep, "gmail")).not.toBeNull();
		expect(await state.get(gmailRep, "calendar")).not.toBeNull();
		expect(await scopeOf()).toBe(SYNC_SCOPES.join(" "));
	});
