...
export function GoogleConnection({
	...
	const setImportSince = useMutation(
		trpc.google.setImportSince.mutationOptions({
			onSuccess: () => cache.google({ settle: "record" }),
			onError: (error) => toast.error(errorMessage(error.message)),
		}),
	);
				...
				{!hasRefreshToken ? (
					...
					failing.map((source) => {
						const { summary, url } = explain(
							source.lastError ?? t("Google needs reconnecting."),
						);
					...
					<div className="-ml-2 flex flex-wrap items-center gap-1 text-muted-foreground">
						...
						<Button variant="ghost" size="xs" asChild>
							<Link
								...
								rel="noreferrer"
							>
								{t("Manage in your Google account")}
							</Link>
						</Button>
					</div>
				</CardFooter>
