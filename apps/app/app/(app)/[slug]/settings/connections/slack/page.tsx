	...
	return status.connected ? (
		...
		<ConnectionPage centered className="max-w-(--container-page)">
			...
			<SlackScopeGroups
				...
				title={t("What you are handing over")}
				withheld={[]}
			/>
			<PlainList
	...
	return (
		<ConnectionPage>
			...
			<SlackScopeGroups
				...
				title={t("What this workspace granted")}
			...
			<section className="flex flex-col gap-3 border-y px-(--spacing-block-inline) py-5">
				<div className="flex items-end justify-between gap-4">
					<div>
						<h2 className="font-medium text-sm">
							{t("Agents that use Slack")}
						</h2>
						<p className="text-muted-foreground text-xs">
							{t("Built in chat, not here. Open one to change it.")}
						</p>
					</div>
					<NewAgentDialog>
						<Button size="sm">{t("New agent")}</Button>
					</NewAgentDialog>
				</div>
				<div className="flex flex-col divide-y rounded-lg border">
					{agents.length === 0 ? (
						<p className="px-(--spacing-block-inline) py-4 text-muted-foreground text-sm">
							{t("No deployed agents use Slack yet.")}
						</p>
					) : null}
					{agents.map(
						(agent: {
					...
					<Link
						...
						href={`/${slug}/chat`}
					>
						{t("Describe another agent in chat")}
					</Link>
				</div>
			</section>
			<div className="flex items-center justify-between gap-4 px-(--spacing-block-inline)">
				<p className="text-sm">
					{status.people.reviewed === 0
						? t("No workspace people have been reviewed yet.")
						: t("{matched} of {reviewed} reviewed people are matched.", {
	...
	return (
		<div className="px-(--spacing-block-inline)">
			<Alert variant="warning">
				...
				<AlertTitle>
					{privateChannels
						? t("{brand} cannot reach private channels", { brand: BRAND.name })
						: missing.length === 1
							? t("Slack held back 1 permission")
							: t("Slack held back {count} permissions", {
									count: missing.length,
								})}
				</AlertTitle>
				<AlertDescription>
					<span>{t("Reconnect to ask again. You lose nothing.")}</span>
