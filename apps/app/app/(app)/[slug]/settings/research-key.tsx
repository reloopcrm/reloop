...
export function ResearchKey() {
	...
	const save = useMutation(
		trpc.settings.setResearchKey.mutationOptions({
			onSuccess: async () => {
				...
				setDraft("");
				toast.success(t("Context API key saved."));
			},
	...
	const { configured, hint, skipped } = key.data;

	return (
		<Card>
			<CardHeader>
				<CardTitle>{t("Company research")}</CardTitle>
				<CardDescription>
					{t(
					...
					<FieldGroup>
						<Field>
							<div className="flex items-center justify-between gap-3">
								<FieldLabel htmlFor={keyId}>{t("Context API key")}</FieldLabel>
								<StatusIndicator
									size="sm"
									tone={
										configured ? "success" : skipped ? "neutral" : "warning"
									}
									label={
										configured
											? t("Connected")
											: skipped
												? t("Reading websites instead")
												: t("Not connected")
									}
								/>
							</div>
							<Input
								...
								placeholder={hint ?? t("Paste the key")}
								autoComplete="off"
								autoCapitalize="off"
								autoCorrect="off"
								spellCheck={false}
