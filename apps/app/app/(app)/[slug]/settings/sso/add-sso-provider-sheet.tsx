	...
	const register = useMutation(
		trpc.sso.register.mutationOptions({
			onSuccess: async (provider) => {
				await cache.sso();
				toast.success(t("{name} saved.", { name: provider.name }));
				await setOpen(null);
				setValues(EMPTY);
			},
	...
	return (
		<Sheet open={open} onOpenChange={(next) => setOpen(next || null)}>
			...
			<SheetContent side="right">
				<SheetHeader>
					<SheetTitle>{t("Add an identity provider")}</SheetTitle>
					<SheetDescription>
						{t("Configure an OpenID Connect provider.")}
					</SheetDescription>
				</SheetHeader>
				...
				>
					<FieldGroup>
						<Field>
							...
							/>
							<FieldDescription>
								{t("Names the sign-in button. Cannot be changed later.")}
							</FieldDescription>
						</Field>

						<Field>
							<FieldLabel htmlFor={issuerId}>{t("Issuer URL")}</FieldLabel>
							<Input
								id={issuerId}
								type="url"
								value={values.issuer}
								...
								required
							/>
							<FieldDescription>{t("Where discovery lives.")}</FieldDescription>
						</Field>

						<Field>
							<FieldLabel htmlFor={domainId}>{t("Email domain")}</FieldLabel>
							<Input
								id={domainId}
								value={values.domain}
							...
							/>
							<FieldDescription>
								{t("Comma-separate several.")}
							</FieldDescription>
						</Field>

						<Field>
							<FieldLabel htmlFor={clientIdId}>{t("Client ID")}</FieldLabel>
							<Input
								id={clientIdId}
						...
						<Field>
							...
							<Input
								...
								required
							/>
							<FieldDescription>{t("Never shown again.")}</FieldDescription>
						</Field>

						<Field>
							<FieldLabel htmlFor={redirectId}>{t("Redirect URI")}</FieldLabel>
							<InputGroup>
							...
							<FieldDescription>
								{t("Add this at your provider before saving.")}
							</FieldDescription>
						</Field>
					</FieldGroup>
				</form>
