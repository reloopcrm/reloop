...
export function OnboardingForm({ placeholder }: { placeholder: string }) {
	...
	return (
		...
		>
			<FieldGroup>
				<Field>
					<FieldLabel htmlFor={nameId}>{t("Company name")}</FieldLabel>
					<Input
						id={nameId}
						name="name"
						value={name}
				...
				<Field>
					<FieldLabel htmlFor={slugId}>{t("Workspace URL")}</FieldLabel>
					<InputGroup>
						<InputGroupAddon>
					...
					<FieldDescription>
						{t("Your team will use this address to open the CRM.")}
					</FieldDescription>
				</Field>
