		...
		<main className="flex min-h-0 flex-1 flex-col">
			<header className="flex h-12 shrink-0 items-center gap-3 border-b px-5">
				<h1 className="min-w-0 flex-1 truncate font-medium text-sm">
					{conversation.agent?.name ?? conversation.title ?? t("Agent builder")}
				</h1>
				<span className="text-muted-foreground text-xs">{t("Read-only")}</span>
			</header>
							...
							<MessageScrollerItem messageId="shared-chat-notice">
								<div className="rounded-lg border bg-card px-4 py-3 text-sm">
									<p className="font-medium">
										{t("Shared by {name}", { name: conversation.ownerName })}
									</p>
				...
				{working ? (
					<span className="flex shrink-0 items-center gap-2 text-muted-foreground text-xs">
						<Icon
							...
							motion="none"
						/>
						<span className="sr-only">{t("Working in background")}</span>
						<span aria-hidden="true" className="hidden sm:inline">
							{t("Working in background")}
						</span>
					</span>
				) : null}
			</div>
			<Button asChild variant="ghost" size="icon-sm">
				<Link href={workspaceUrl("/chat")} aria-label={t("Start a new chat")}>
					<Icon icon={Add} />
				</Link>
			</Button>
...
function UserSubmission({
	...
	const messageText = message.text ?? t("Message unavailable");
	const command =
			...
			>
				{response ? (
					<div className="flex items-center gap-1.5 text-muted-foreground text-xs">
						...
						<span>{t("Answer to follow-up")}</span>
					</div>
				) : null}
				{submission.commandType === "CREATE_AGENT" ? (
					<div className="flex flex-wrap gap-1">
						<ChatCommandChip label={t("Create agent")} icon={Application} />
					</div>
				) : null}
	...
	return (
		<div className="w-full max-w-sm border-ring/50 border-l-2 bg-muted/40 px-3 py-2.5">
			<div className="flex items-center justify-between gap-3 text-xs">
				<span className="font-medium">{t("Follow-up")}</span>
				<span className="text-muted-foreground">
					{answered ? t("Answered") : t("Waiting for your answer")}
				</span>
			</div>
	...
	return (
		<div className="flex h-7 items-center">
			<Button
				variant="ghost"
				size="icon-xs"
				aria-label={t("Copy response as Markdown")}
				onClick={() => {
					void navigator.clipboard.writeText(markdown);
					toast.success(t("Response copied as Markdown."));
				}}
			>
				<Icon icon={Copy} />
			</Button>
	...
	return (
		<div className="flex h-7 items-center gap-0.5">
			...
			<Button
				variant="ghost"
				size="icon-xs"
				aria-label={t("Rate response helpful")}
			...
			<Button
				variant="ghost"
				size="icon-xs"
				aria-label={t("Rate response not helpful")}
...
function BuildingAgentCard({
	...
	const stop = useAsyncAction({
		...
		onSuccess: () => toast.success(t("Stop requested.")),
		onError: () => toast.error(t("The agent could not be stopped. Try again.")),
	});
							...
							>
								...
								{active && writingPath ? (
									<p className="pl-7 font-mono text-muted-foreground text-xs">
										{t("Writing {path}", { path: writingPath })}
									</p>
								) : null}
							</li>
						);
	...
	return (
		<div className="flex flex-col gap-5">
			<p className="max-w-[640px] text-pretty text-sm leading-5">
				{t("Your private draft is ready to review.")}
			</p>
			<AgentCardShell name={manifest.name ?? agent.name} status={t("Private")}>
				<div className="flex flex-col gap-2 p-4">
					...
					<ReviewRow label={t("Scope")}>
						<AgentScopeBadges
							...
							fallback={t("Bounded CRM read access")}
						/>
					</ReviewRow>
				</div>
				<AgentCardFooter
					note={t("Sandboxed · credentials never enter the sandbox")}
				>
		...
		<div className="flex flex-col gap-[18px]">
			<div className="flex flex-col gap-1">
				<p className="text-sm leading-5">
					{t("{name} is live.", { name: agent.name })}
				</p>
			...
			<AgentCardShell name={agent.name} status={t("Live")}>
				<div className="flex flex-col gap-2 p-4">
					<ReviewRow
						label={t("Trigger")}
						value={
					...
					<ReviewRow
						label={t("Runs in")}
						value={t("Eve runtime · isolated sandbox")}
					/>
					<ReviewRow
						label={t("Owner")}
						value={t("Team · {name}", { name: agent.createdBy.name })}
					/>
				</div>
				<AgentCardFooter
					note={t("The chat stays private. The agent is team-owned.")}
				>
			...
			<div>
				...
				{["Add another teammate to the notification"].map((suggestion) => (
					<button
						key={suggestion}
						type="button"
						onClick={() => void onFollowUp(t(suggestion))}
	...
	return (
		<main className="flex flex-1 items-center justify-center p-8">
			<div className="max-w-md text-center">
				<h1 className="font-medium text-lg">{t("Chat unavailable")}</h1>
				<p className="mt-2 text-muted-foreground text-sm">
					{t("This chat does not exist or you do not have access to it.")}
				</p>
				<Button asChild variant="outline" className="mt-5">
					<Link href={workspaceUrl("/chat")}>{t("Start a new chat")}</Link>
				</Button>
			</div>
		</main>
	);
...
function manifestOf(manifest: AgentManifestSummary, t: Translate) {
	return {
			...
			manifest.triggers
				.map((trigger) =>
					trigger.type === "MANUAL"
						...
						: compactSummary(
								trigger.summary,
								trigger.type === "EVENT" ? t("On CRM event") : t("On schedule"),
							),
				)
				.join(" · ") || t("On demand"),
		looksAt: textOf(
			manifest.dataScope.summary,
			t("CRM records in the approved scope"),
		),
		action: compactSummary(
			manifest.actions[0]?.summary,
			t("Perform the requested team action"),
		),
