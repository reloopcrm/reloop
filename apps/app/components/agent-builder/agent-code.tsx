	...
	return (
		<EditProvider createEditor={createEditor}>
			<section className="flex flex-col gap-3.5">
				<div className="flex items-end justify-between gap-4">
					<div>
						...
						<p className="text-muted-foreground text-sm">
							{t("What the agent actually runs.")}
						</p>
					</div>
					...
					<div className="flex items-center gap-2">
						{file?.previousContent ? (
							<Button
								...
								variant="outline"
							>
								{showDiff ? t("Hide changes") : t("Changes")}
							</Button>
						) : null}
						{canManage ? (
							<Button
			...
			<SaveBar
				description={
					changed.length === 1
						? t("{count} file changed. Saving writes a new revision.", {
								count: changed.length,
							})
						: t("{count} files changed. Saving writes a new revision.", {
								count: changed.length,
							})
				}
				open={changed.length > 0}
				title={t("Unsaved code")}
			>
