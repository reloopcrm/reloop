			...
			<div className="flex h-7 items-center justify-between">
				...
				>
					<AsyncButtonContent
						...
						errorLabel={<span className="sr-only">{t("Send failed")}</span>}
					>
						<Icon icon={ArrowUp} />
					</AsyncButtonContent>
				</Button>
...
function ComposerEditor({
	...
	const placeholder =
		mode === "home"
			? t("Ask about your CRM or automate a task…")
			: t("Send a message");
	const commit = () => {
	...
	return (
		<TokenField
			...
			aria-label={t("Message the agent builder")}
			aria-multiline="true"
		...
		>
			<PopoverTrigger asChild>
				<Button
					variant="ghost"
					size="icon-sm"
					aria-label={t("Tag CRM records and integrations")}
					disabled={disabled}
				>
					<Icon icon={Add} />
				</Button>
				...
				<div className="border-b p-2">
					<InputGroup>
						<InputGroupInput
							...
							placeholder={t("Search CRM")}
							aria-label={t("Search CRM")}
							disabled={disabled}
						/>
						<InputGroupAddon>
...
function AttachmentPicker({
	...
	const addFiles = async (files: FileList | null) => {
		...
		try {
			...
			toast.error(t("Those files could not be attached. Try again."));
		} finally {
		...
		>
			<PopoverTrigger asChild>
				<Button
					variant="ghost"
					size="icon-sm"
					aria-label={t("Open slash commands")}
					disabled={disabled}
...
async function readFiles(
	...
	for (const file of Array.from(files).slice(0, 5)) {
		...
		if (file.size > 2_000_000) {
			toast.error(t("{name} is larger than 2 MB.", { name: file.name }));
			continue;
		}
		if (file.name.length > 180) {
			toast.error(t("That file name is too long."));
			continue;
		}
		if (file.type.length > 120) {
			toast.error(
				t("{name} has an unsupported file type.", { name: file.name }),
			);
			continue;
		}
	...
	const attachments = await Promise.all(
		acceptedFiles.map(async (file): Promise<BuilderUploadAttachment | null> => {
			try {
				...
				toast.error(t("{name} could not be read.", { name: file.name }));
				return null;
			}
		}),
	);
