		...
		<div className="flex min-w-0 flex-col gap-4 sm:gap-6">
			...
			{visible.map((run) => (
				...
				>
							...
							<span className="flex min-w-0 items-center justify-between gap-3 font-mono text-muted-foreground text-xs sm:shrink-0 sm:justify-start sm:gap-4">
								...
								<span>
									{run.actions.length === 1
										? t("{count} external action", {
												count: run.actions.length,
											})
										: t("{count} external actions", {
												count: run.actions.length,
											})}
								</span>
								<Icon
					...
					{expanded === run.id ? <ExpandedRun run={run} t={t} /> : null}
				</div>
			))}
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>{t("Stop this run?")}</AlertDialogTitle>
						<AlertDialogDescription>
							{t(
					...
					<AlertDialogFooter>
						<AlertDialogCancel>{t("Keep running")}</AlertDialogCancel>
						<AlertDialogAction
							variant="destructive"
							onClick={() => {
...
function ExpandedRun({ run, t }: { run: RunRow; t: Translate }) {
	const locale = useLocale();
	...
	return (
		<div className="min-w-0 border-t">
			<div className="grid grid-cols-2 gap-x-4 gap-y-3 border-b bg-background px-4 py-3 sm:min-h-[58px] sm:grid-cols-4 sm:items-center sm:gap-0 sm:px-5 sm:py-2">
				<RunMeta label={t("Trigger")} value={t(humanStatus(run.triggerType))} />
				<RunMeta
						...
						>
							...
							<span className="min-w-0 flex-1">
								...
								<span className="block wrap-break-word text-muted-foreground text-xs">
									...
									{t(humanStatus(entry.action.status))}
				...
				{run.eventsTruncated ? (
					<div className="flex min-h-9 items-center border-t px-4 py-2 text-warning text-xs sm:px-5">
						{t(
							...
							{ shown: events.length, total: run.totalEvents },
						)}
					</div>
				) : null}
			</div>
			...
			<div className="min-w-0 overflow-hidden rounded-lg border bg-card">
				...
				{visible.length === 0 ? (
					<p className="px-5 py-12 text-center text-muted-foreground text-sm">
						{t("No changes match this filter.")}
					</p>
				) : null}
			</div>
		</div>
...
function formatTime(value: string, locale: Locale): string {
	return dateFormat(locale, TIME_OPTIONS).format(new Date(value));
}
...
function duration(
	...
	if (!startedAt) return "-";
	if (!finishedAt) return t("In progress");
