	...
	const revoke = useMutation(
		trpc.conversations.revokeShare.mutationOptions({
			onSuccess: async (_result, input) => {
				...
				await invalidate();
				toast.success(t("Chat link revoked."));
			},
			onError: (error) => toast.error(errorMessage(error.message)),
		}),
	);
					...
					{status.isPending ? (
						...
						>
							<p className="text-muted-foreground text-sm">
								{t("Sharing status could not be loaded.")}
							</p>
							<Button
								variant="outline"
								size="sm"
						...
						<>
							...
							<ShareChoice
								...
								label={t("Anyone in {brand} with the link", {
									brand: BRAND.name,
								})}
								detail={t("Read-only")}
								disabled={createAction.pending || revokeAction.pending}
								...
								<div className="flex h-9 items-center gap-3 rounded-full border bg-card py-0.75 pr-0.75 pl-4">
									<span className="min-w-0 flex-1 truncate font-mono text-xs">
										{shareToken
											...
											: t("An active read-only link exists")}
									</span>
									<Button
										variant="outline"
										size="sm"
									...
									>
										<AsyncButtonContent
											...
											pendingLabel={
												shareToken ? t("Copying") : t("Creating link")
											}
											successLabel={
												shareToken ? t("Copied") : t("Link created")
											}
										...
										>
											...
											{shareToken ? t("Copy link") : t("Replace link")}
										</AsyncButtonContent>
									</Button>
								</div>
							) : null}
