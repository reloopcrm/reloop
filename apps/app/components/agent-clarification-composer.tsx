		...
		try {
			...
			setTransportError(
				error instanceof Error && error.message.trim()
					? error.message
					: t("Unable to submit. Check your connection and try again."),
			);
		}
	};
	...
	return (
		...
		>
			<QuestionnaireItem name="response" required>
				...
				<QuestionnaireDescription>
					{options.length > 0
						? t("Choose an answer, then submit.")
						: t("Add the detail the agent needs to continue.")}
				</QuestionnaireDescription>
				<QuestionnaireChoices>
					{options.map((option) => (
						<QuestionnaireChoice
					...
					{showFreeform ? (
						<QuestionnaireInput
							aria-label={
								options.length > 0 ? t("Another answer") : t("Your answer")
							}
							placeholder={t("Add the detail the agent needs")}
							disabled={pending}
						/>
					) : null}
				</QuestionnaireChoices>
