...
export function ConversationPicker({
	...
	const t = useT();
	const label = current?.title ?? t("New conversation");
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="start" className="w-72">
					{conversations.length === 0 ? (
						<DropdownMenuItem disabled>{t("Nothing yet")}</DropdownMenuItem>
					) : (
...
function Forget({
	...
	return (
		...
		>
			<Icon icon={TrashCan} />
			<span className="sr-only">{t("Delete this conversation")}</span>
		</Button>
	);
}
