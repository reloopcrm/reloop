	...
	const create = useMutation(
		trpc.contacts.create.mutationOptions({
			onSuccess: async (contact) => {
				...
				toast.success(t("{name} added.", { name: contact.firstName }));
				onDone();
			},
...
export function AttachDealContact({
	...
	const attach = useMutation(
		trpc.deals.attachContact.mutationOptions({
			onSuccess: async (attached) => {
				...
				toast.success(
					person
						? t("{name} is on the deal.", { name: contactName(person) })
						: t("Added to the deal."),
				);
				onDone();
			},
	...
	const placeholder = options.isPending
		? t("Loading…")
		: nobody
			? t("Everybody at {company} is already on it", { company: companyName })
			: t("Choose somebody");

	return (
		<QuickAddForm
			submitLabel={t("Add to deal")}
...
export function QuickAddDeal({
	...
	const create = useMutation(
		trpc.deals.create.mutationOptions({
			onSuccess: async (deal) => {
				...
				toast.success(t("{name} created.", { name: deal.name }));
				onDone();
			},
			onError: (error) => toast.error(errorMessage(error.message)),
		}),
	...
	const submit = () => {
		if (!owner) {
			toast.error(t("Could not work out who should own this deal."));
			return;
		}
	...
	return (
		...
		>
			...
			<Field>
				<FieldLabel htmlFor={closeId}>{t("Expected close")}</FieldLabel>
				<DatePicker
					id={closeId}
					value={closeDate}
