				...
				{footer.more > 0 ? (
					<div className="flex items-center justify-between px-4 py-3 text-xs">
						<span className="text-muted-foreground">
							{t("{count} more queued", { count: footer.more })}
						</span>
	...
	return (
		<Collapsible className="border-t" open={shown} onOpenChange={setShown}>
			<CollapsibleTrigger className="flex w-full cursor-pointer items-center justify-between px-4 py-3 text-xs hover:bg-muted">
				<span className="text-muted-foreground">
					{total === 1
						? t("The agent looks at 1 record again later")
						: t("The agent looks at {count} records again later", {
								count: total,
							})}
				</span>
			...
			<CollapsibleContent className="max-h-60 overflow-y-auto border-t">
				{rows.map((row) => (
					...
					>
						...
						<span className="shrink-0 text-muted-foreground text-xs">
							{row.due.count === null
								? t(row.due.text)
								: t(row.due.text, { count: row.due.count })}
						</span>
					</button>
				))}
				...
				{total > rows.length ? (
					<div className="px-4 py-3 text-muted-foreground text-xs">
						{t("{count} more are waiting but not listed here", {
