			...
			{channels.map((channel) => {
				...
				return (
					...
					>
						{onSelect && selectable ? (
							<button
								aria-label={t("Choose #{name}", { name: channel.name })}
...
function describe(
	...
	const people =
		channel.memberCount === null
			? ""
			: ` · ${t(channel.memberCount === 1 ? "1 person" : "{count} people", { count: channel.memberCount })}`;
	if (channel.isMember)
		return t("{brand} is in{people}", { brand: BRAND.name, people });
	if (!channel.classified)
		return t("Not read from Slack yet{people}", { people });
	if (!channel.isPrivate)
		return t("{brand} can join this one{people}", {
			brand: BRAND.name,
			people,
		});
	if (canInviteItself)
		return t("Private. {brand} joins as you{people}", {
			brand: BRAND.name,
			people,
		});
	if (channel.inviteRequestedAt)
		return t("Private. Waiting on an invite{people}", { people });
	return t("Private. Someone inside has to invite {brand}{people}", {
		brand: BRAND.name,
		people,
	});
}
