...
const VERBS: ToolVerbs = {
	...
	list_win_back_candidates: "Looked for people worth winning back",
	read_contact_memory: "Read what the CRM remembers about them",
	read_playbook: "Read how you usually do business",
...
export function describe(part: EveMessagePart): string {
	...
	return reason === null ? verb : `${verb}${REASON_SEPARATOR}${reason}`;
}

export function splitLabel(item: {
	label: string;
	tool: string;
}): { verb: string; reason: string | null } | null {
	const verb = VERBS[item.tool];
	if (verb === undefined || !item.label.startsWith(verb)) return null;

	const rest = item.label.slice(verb.length);
	if (rest === "") return { verb, reason: null };
	if (!rest.startsWith(REASON_SEPARATOR)) return null;

	return { verb, reason: rest.slice(REASON_SEPARATOR.length) };
}
