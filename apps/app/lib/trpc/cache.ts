...
export type CrmCache = {
	...
	profile(options?: Options): Promise<void>;
...
export function useCrmCache(): CrmCache {
	...
	return {
		...
		winBack: (options) =>
			run(
				[
					trpc.reactivation.rules.queryKey(),
		...
		settings: (options) =>
			run(
				[
					...
					trpc.settings.agentLanguage.queryKey(),
