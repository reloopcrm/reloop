...
`bun run db:test` creates a missing test database and applies migrations.
Use it only for an explicitly authorized disposable database.
The database name contains letters, digits, and underscores, and ends in `_test`.

An existing database must have readable migration history and match `schema.prisma`.
Foreign migrations, schema drift, and failed schema checks stop preparation with a diagnostic.
The helper never drops a database or terminates connections. It refuses `--reset`.
Review schema differences separately. Do not reset an existing database to fix a test failure.

The full integration suites delete fixture rows. They require explicit authorization for a separate disposable database.
A `_test` suffix alone does not provide authorization.

### API start and isolated checks

Run `bun run start` inside `apps/api` to compile and start the API.
`start:prod` runs the existing build. `dev` compiles and watches API source and shared packages.
Tests load `test/classic-decorators.ts` through `test/setup.ts` to preserve classic decorators and constructor metadata.
Direct `bun test` calls need `--preload ./test/classic-decorators.ts` for decorated API files.

Root `bun run dev` still runs `dev:prepare` and applies migrations.
Do not use that command for a check that prohibits database changes.

`packages/db/scripts/check-contact-limit.ts` prepares a separate schema and checks competing contact inserts.
It reads `TEST_DATABASE_URL` and requires `--authorized-disposable-database`.
Run it only after explicit authorization for that database.
It preserves its fixture schema and records. It never removes existing records.

Run `bun run --filter=api test:security` for the isolated regression suite.
It compiles the API and runs an explicit list of tests.
Database URLs point to a closed local test port. Database and provider calls use test doubles.
The command runs no database preparation, migrations, synchronization, or cleanup.
