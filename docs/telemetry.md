...
**Where it goes** is `POSTHOG_KEY` in the root `.env`, read through
`packages/telemetry/src/project.ts`. Without that key no client is created and nothing can be
sent, so a self-hosted install is silent until you point it at your own PostHog project. A
`phc_` key is write-only: it sends events and reads nothing back, so it is a public identifier
and not a secret. `POSTHOG_HOST` and `POSTHOG_UI_HOST` only need changing for a self-hosted
PostHog or for the US cloud. Nothing is sent while `NODE_ENV` is `test`, so a test run cannot
emit.
...
- **Server side only, in everything you run.** `posthog-node` in `apps/api` and `apps/agent`.
  ...
  public landing page loads no analytics either.
...
| `agent_model_id` | The chat model of the provider chosen on Settings → AI, e.g. `openai/gpt-5.6-luna` |
...
| `packages/telemetry/src/project.ts` | The key, the ingest host and the UI host, read from `POSTHOG_KEY`, `POSTHOG_HOST` and `POSTHOG_UI_HOST`. No key means nothing is sent |
