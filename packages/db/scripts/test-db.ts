...
if (process.argv.includes("--reset")) {
	fail(["Database reset is disabled. Existing data stays unchanged."]);
}
...
if (!process.env.TEST_DATABASE_URL) {
	console.log(
		`Set TEST_DATABASE_URL in the root .env to the connection URL for ${name}.`,
	);
}
...
async function create(target: string, database: string): Promise<void> {
	...
	try {
		...
		if (existing.rowCount) {
			const reason = await stale(target, database);
...
async function stale(target: string, database: string): Promise<string | null> {
	...
	if (applied === null) return `${database} has no readable migration history`;
...
function drifted(target: string): boolean {
	...
	if (result.error || (result.status !== 0 && result.status !== 2)) {
		fail([
			"Schema comparison fails. Database preparation stops.",
