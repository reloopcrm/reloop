...
const CONFIG: TrackingConfig = {
	...
	cookieDays: 395,
	hosts: [
		{ host: "reloop.de", scope: "SITE_AND_SUBDOMAINS" },
...
describe("the allow-list", () => {
	test("covers subdomains only where the scope says so", () => {
		expect(hostAllowed("reloop.de", CONFIG)).toBe(true);
		expect(hostAllowed("docs.reloop.de", CONFIG)).toBe(true);
	...
	test("never matches a host that merely ends the same way", () => {
		expect(hostAllowed("notreloop.de", CONFIG)).toBe(false);
		expect(hostAllowed("reloop.de.evil.com", CONFIG)).toBe(false);
	});
...
describe("the domain an event belongs to", () => {
	test("is the parent when the scope covers subdomains", () => {
		expect(matchedHost("docs.reloop.de", CONFIG)?.host).toBe("reloop.de");
		expect(matchedHost("reloop.de", CONFIG)?.host).toBe("reloop.de");
	});
	...
	test("prefers the host's own row to the parent that also covers it", () => {
		const both: TrackingConfig = {
			...
			hosts: [
				{ host: "reloop.de", scope: "SITE_AND_SUBDOMAINS" },
				{ host: "docs.reloop.de", scope: "EXACT_HOST" },
			],
		};

		expect(matchedHost("docs.reloop.de", both)?.host).toBe("docs.reloop.de");
		expect(matchedHost("blog.reloop.de", both)?.host).toBe("reloop.de");
	});
});
...
describe("the origin check", () => {
	test("accepts an allowed origin and refuses the rest", () => {
		expect(originAllowed("https://docs.reloop.de", CONFIG)).toBe(true);
...
describe("the submission dedupe key", () => {
	test("collapses the same form inside one minute", () => {
		...
		const parts = { host: "reloop.de", path: "/pricing", email: "a@b.com" };
