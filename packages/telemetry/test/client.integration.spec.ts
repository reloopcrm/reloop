...
afterEach(() => {
	...
	restore("POSTHOG_KEY", real.posthogKey);
});
...
describe("capture", () => {
	...
	it("sends nothing and reports the failure when no PostHog key is set", async () => {
...
describe("milestone", () => {
	...
	it("leaves the step unreached when no PostHog key is set", async () => {
		delete process.env.POSTHOG_KEY;
		resetTelemetryClient();
